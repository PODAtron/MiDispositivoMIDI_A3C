...
void readPADs() {
  for (int i = 0; i < LEDPAD_NUM; i++) { // PAD Button Iterator
    ...
    for (int j = 0; j < LEDPAD_EXT; j++) { // PAD Channel Iterator
      ...
      //Metido mio aparte de pablo
      if (padRead[i] < PAD_THR) { //Si esta pulsado
        if (lastRead[i][currentPage] == 0) { //Si no se ha enviado ya

          stepStatus[i][currentPage] = !(stepStatus[i][currentPage]);
          //Serial.println(stepStatus[j][i][currentPage]);
...
/* Funtion void execPADs()
    Read the PAD matrix and executes actions
*/
// //unused by now
// void execPADs(){
//   for(int i=0; i<LEDPAD_NUM; i++){ // PAD Button Iterator
//      for(int j=0; j<LEDPAD_EXT; j++){ // PAD Channel Iterator
//        if(padRead[j][i] < PAD_THR){
//          //Serial.print("\nPAD CH: ");
//          //Serial.print(j);
...
//          ledPad[j][i][1] = 255; // Set Pad LED BLUE
//          padRead[j][i] = 700; // Clean Sticky Reads
//        }else{
//          ledPad[j][i][1] = 0; // Set PAD LED OFF
//          //padRead[j][i] = 0;
//        }
//      }
//  }
...
void updateLEDs() {
  for (int i = 0; i < LEDPAD_NUM; i++) { // PAD Button Iterator
    for (int j = 0; j < LEDPAD_EXT; j++) { // PAD Channel Iterator
      ...
      //ledPadStripe.setPixelColor((i*(j+1)),ledPad[j][i][0],ledPad[j][i][1],ledPad[j][i][2]); // Heavy Stuff
    }
  }
...
//Un poquillo guarrete. es para pruebas
void timerIsr()
{
  Serial.println("Tick");
  uint8_t page = 0;
